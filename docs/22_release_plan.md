...
| 7 | `plan.md` | **Stays untracked.** Reversed from the original recommendation to track it, on review of its contents: parts of it are comparative working notes that are not licence-cleared for redistribution, and the document itself flags that reuse needs clearance. It therefore stays a local design history. The 130 dangling citations were redirected to the tracked `docs/` guide instead, and `ci/check_doc_refs.py` now prevents new ones. |
...
- [x] `README.md` rewritten for users, 637 lines: three install routes, a
      quickstart that starts with the native zero-dependency path, the TSV column
      tables, the q-unit table with both traps, benchmark numbers each carrying
      its row unit and selection column, a status-of-experimental-features
      section, hardware sizing, badges. One old claim ("97 to 98% sequence
      concordance with DIA-NN") was removed rather than restated, being
      unattributable in the tracked tree.
- [x] `docs/23_cli_reference.md` (665 lines, all 21 subcommands, `--config`
      support derived per subcommand) and `docs/24_config_reference.md` (732
      lines, 17 structs, 166 fields, 21 enums, 20 gated fields, 0 unresolved
      defaults), both generated by `ci/gen_*_reference.py` with a `--check` mode
      wired into CI. The environment-variable table is generated too: 47 read
      across engine and sidecars, plus the 11 the code sets.
- [x] `docs/19_getting_started.md` rewritten: opens with `ci/smoke.sh`, which
      needs nothing but the repository, then the two shipped environment specs and
      `"auto"` resolution, then the runs that need supplied data. Zero
      machine-specific paths remain.
- [x] `docs/15`: the TSV columns are documented, with the row-unit-versus-filter
      trap stated, and the schema-version registry drift fixed.
- [x] `docs/14`: test counts, per-file table, the five CI jobs, the release and
      Docker workflows, the MSRV, the `validate()` counts and the `doctor`
      behaviour all re-derived from the current files.
...
- [ ] `CITATION.cff`: BLOCKED on the author list, which is not derivable from the
      repository and must not be guessed. Issue and PR templates not written.
...
- [ ] A second smoke job with a conda env for the sidecar path; macOS coverage for
      the smoke job.
- [x] Python tests (`tests/python`, run in CI): 71 tests over the worker
      contracts, including the `mbr_worker` q-column regression, out-of-fold
      coverage for the rescorers, the library-recipe encoding contracts and the
      DeepLC import order. Verified across three interpreters so every test
      executes somewhere: 59 pass with numpy and pyarrow alone, 65 with mokapot
      and DeepLC present, and the 5 `nn_torch` tests pass against a working torch.
      They found a real bug: the library helpers wrote `large_string` parquet on
      pandas 3, which the engine rejects, so every library they built on a current
      pandas was unreadable. Fixed in `scripts/_lib_io.py`.
