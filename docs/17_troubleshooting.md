...
| No `peptides.tsv` or `proteins.tsv` anywhere under a `run-experiment` output tree | `run-experiment` never calls the report stage | Run `mumdia report` manually against each split scored table, or count from the parquet |
