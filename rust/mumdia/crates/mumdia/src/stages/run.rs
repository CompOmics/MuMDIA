...
pub fn run(p: RunParams) -> Result<()> {
        ...
        > 1
    {
        ...
        let (n, seed_ms2) = search_seed::run_returning_scans(search_seed::SearchSeedParams {
        ...
        let lent_ms2: Option<Vec<mumdia_core::types::Ms2Scan>> = seed_ms2.filter(|_| !rt_sidecar);
        ...
        // Extract takes the MS2 and MS1 scans together or not at all
        // (`ExtractParams::scans`), so a lent MS2 comes with this run's MS1, decoded
        // here (it is a small fraction of the MS2).
        let lent_ms1: Option<Vec<crate::spectra::Ms1Scan>> = match lent_ms2 {
            Some(_) => Some(crate::spectra::load_ms1(&co.ms1)?),
            None => None,
        };
        let (npsm, nchr) = extract::run(extract::ExtractParams {
            ...
            scans: lent_ms2
                .as_deref()
                .zip(lent_ms1.as_deref())
                .map(|(ms2, ms1)| extract::SharedScans { ms2, ms1 }),
            ms2: &co.ms2,
            library_precursors: &lib_p,
            library_fragments: &lib_f,
            run_windows: &windows,
