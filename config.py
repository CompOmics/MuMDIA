...
def load_config_from_json(json_path: str) -> 'MuMDIAConfig':
    ...
    filtered_data = {k: v for k, v in config_data.items() if not k.startswith('_comment')}
...
class MuMDIAConfig:
    ...
    def get_mumdia_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "get_first_entry": [  # Default list
                "psm_id", "filename", "scannr", "peptide", "num_proteins",
                "proteins", "expmass", "calcmass", "is_decoy", "charge",
                "peptide_len", "missed_cleavages"
            ],
    ...
    def from_args(cls) -> 'MuMDIAConfig':
        ...
        parser = argparse.ArgumentParser(description="MuMDIA: Simplified Configuration System")
        
        # File arguments
        parser.add_argument("--config_file", default="configs/config.json", help="Path to JSON configuration file")
        ...
        parser.add_argument("--training_fdr", type=float, default=0.01, help="Training FDR")
        parser.add_argument("--final_fdr", type=float, default=0.01, help="Final FDR")
        parser.add_argument("--model_type", choices=["xgboost", "nn", "percolator"], default="xgboost", help="ML model type")
